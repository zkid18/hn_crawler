...
async def parse_main_page(main_page):
    '''Parse the hackernews main page

    Args:
        main_page(str): url to the main page
    ...
    On the crawling stage,
    save the discussion_link id and the last_rank to the cache storage.
...
async def parse_article_page(article_dir, article_url):
    ...
    '''
    logging.info(
        'Parsing the article page:'
        '{} searching for the links in the comments'.format(article_url)
    )
...
async def main(output_dir):
    ...
    logging.info(
        'Parsed the main page: {} new articles'.format(len(articles_url))
    )
    ...
    for article_url in articles_url:
        ...
        if os.path.isdir(article_dir):
            logging.info("Page {} was already parsed".format(article_dir))
            continue
        article_dir.mkdir(parents=True, exist_ok=True)

        task = asyncio.create_task(
            parse_article_page(article_dir, URL+article_url)
            )
        tasks.append(task)
...
def parse_arguments():
    ...
    parser.add_argument('-i', '--interval', type=str, default=TIMEOUT_INTERVAL,
                        help='Interval in minutes for the monitoring')
